package com.janosgyerik.practice.oj.adventofcode.y2016.day4;
...
public class SecurityThroughObscurityTest {
    ...
    static class Room {
        ...
        static Room fromLine(String line) {
            int sectorIdStart = line.lastIndexOf('-') + 1;
            int checksumStart = line.lastIndexOf('[') + 1;
            String name = line.substring(0, sectorIdStart - 1);
            int sectorId = Integer.parseInt(line.substring(sectorIdStart, checksumStart - 1));
            String checksum = line.substring(checksumStart, line.length() - 1);
    ...
    boolean isRealRoom(String line) {
        return Room.fromLine(line).isRealRoom();
    }

    @Test
    public void test_real_room_example_1() {
        assertThat(isRealRoom("aaaaa-bbb-z-y-x-123[abxyz]")).isTrue();
    }

    @Test
    public void test_real_room_example_2() {
        assertThat(isRealRoom("a-b-c-d-e-f-g-h-987[abcde]")).isTrue();
    }

    @Test
    public void test_real_room_example_3() {
        assertThat(isRealRoom("not-a-real-room-404[oarel]")).isTrue();
    }

    @Test
    public void test_real_room_example_4() {
        assertThat(isRealRoom("totally-real-room-200[decoy]")).isFalse();
    }

    @Test
    public void test_solvePart1_is_409147() {
        assertThat(solve(puzzleInput)).isEqualTo(409147);
    }

    private int solve(String input) {
        int sum = 0;
        for (String line : input.split("\n")) {
            Room room = Room.fromLine(line);
            if (room.isRealRoom()) {
                sum += room.sectorId;
            }
        }
    ...
    @Test
    public void test_qzmt_rot343_is_very() {
        assertThat(rotate("qzmt", 343)).isEqualTo("very");
    }
    ...
    private String rotate(String s, int n) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            if ('a' <= c && c <= 'z') {
                result[i] = (char)('a' + (s.charAt(i) - 'a' + n) % 26);
            } else {
                result[i] = c;
            }
        }
    ...
    @Test
    public void test_find_north_pole() {
        assertThat(findNorthPole()).isEqualTo(991);
    }

    private int findNorthPole() {
        for (String line : puzzleInput.split("\n")) {
            Room room = Room.fromLine(line);
            if (rotate(room.name, room.sectorId).startsWith("northpole-")) {
                return room.sectorId;
            }
        }
        throw new IllegalStateException("should have found northpole");
    }
}
